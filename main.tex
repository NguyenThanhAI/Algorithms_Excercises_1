...
\begin{document}
    ...
    \begin{loigiai}
        ...
        \item \textbf{Ví dụ 3:} Một đoàn xe gồm $N$ xe tải, đang xếp hàng để qua một chiếc cầu.
...
d            \max\Big( \bold{L}_{i-1, j}, \bold{L}_{i, j-1}, \bold{L}_{i-1, j-1}, \bold{G}_{i, j} - \bold{G}_{i - k, j} - \bold{G}_{i, j - k} + \bold{G}_{i - k, j - k} \Big ) \text{ nếu } i > k \text{ và } j > k \end{cases}
    ...
    \textbf{Bài toán 2:}
    Cho $n$ ma trận $\bold{A}_1, \bold{A}_2, \dots, \bold{A}_n$, ma trận $\bold{A}_i$ có kích thước $d_{i-1} \times d_i$.
    ...
    \begin{algorithm}
        ...
        \For {$i \gets 1$ \textbf{to} $n$} {
            ...
            \For{$j \gets i + 2$ \textbf{to} $n$} {
                ...
                $\bold{L} \lbrack i, j \rbrack \gets min\_val $\;
            }
        }
